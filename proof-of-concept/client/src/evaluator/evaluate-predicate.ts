...
export const getScopeWithValues = (
  scope: Predicate["scope"],
  value: unknown
) => {
  return Object.entries(scope).reduce<Record<string, unknown>>(
    (output, [k, v]) => {
      if (!Array.isArray(v)) {
        output[k] = v;
        return output;
...
export const insertValuesIntoExpress = (
  expression: string,
  scope: { [key in string]: any }
) => {
...
export const evaluatePredicate = (predicate: Predicate, value: any) => {
  ...
  const expression = Object.entries(scopeWithValues).reduce(
    (output, [k, v]) => {
